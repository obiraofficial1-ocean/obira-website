...
function spinCouponWheel() {
  ...
  setTimeout(() => {
    localStorage.setItem('obiraCoupon', prize.code);
    result.textContent = `You won ${prize.rate}% off — code ${prize.code} is applied.`;
...
function applyCoupon() {
  ...
  if (OBIRA_COUPONS[code]) {
    localStorage.setItem('obiraCoupon', code);
    if (message) message.textContent = `${code} applied: ${OBIRA_COUPONS[code]}% off`;
    showNotification(`${code} applied`);
  } else if (message) {
    message.textContent = 'Use OBIRA5, WELCOME8 or GLOW10.';
  }
...
function updateCouponLabels() {
  document.querySelectorAll('[data-coupon-label]').forEach(el => {
    el.textContent = getCoupon() ? `${getCoupon()} · ${getCouponRate()}% off` : 'No coupon applied';
  });
}
...
function renderPairing(product) {
  ...
  host.innerHTML = `
    <div class="pairing-panel">
      <h3>Complete the look · Save ${OBIRA_PAIR_DISCOUNT}%</h3>
      <div class="pair-options">
        <button class="pair-option" onclick="addPairToCart('${pair.id}')">
          <img src="${pair.image}" alt="${pair.name}">
          <span>
            <strong>${pair.name}</strong>
            <small>${formatINR(offer)} pair for both</small>
          </span>
        </button>
      </div>
    </div>
...
function addPairToCart(pairId) {
  const selected = JSON.parse(sessionStorage.getItem('selectedProduct') || 'null');
  if (selected && typeof increaseQty === 'function') increaseQty(selected.id);
  if (typeof increaseQty === 'function') increaseQty(pairId);
  showNotification('Pair added — bundle savings unlocked!');
}
...
function obiraEnhanceCart() {
  ...
  summary.innerHTML = `
    <div class="summary-card">
      <h3>Order summary</h3>
      <p class="summary-row"><span>Subtotal</span><strong>${formatINR(subtotal)}</strong></p>
      <p class="summary-row"><span>Pairing savings</span><strong>-${formatINR(pairSaving)}</strong></p>
      <p class="summary-row"><span>Coupon <small data-coupon-label>${getCoupon() ? `${getCoupon()} · ${getCouponRate()}% off` : 'No coupon applied'}</small></span><strong>-${formatINR(couponSaving)}</strong></p>
      <p class="summary-row"><span>Tax (${OBIRA_TAX_RATE}%)</span><strong>${formatINR(tax)}</strong></p>
      <p class="summary-row"><span>Shipping</span><strong>${formatINR(SHIPPING_COST)}</strong></p>
      <p class="summary-row total"><span>Total</span><strong>${formatINR(total)}</strong></p>
...
window.addEventListener('load', () => {
  ...
  setTimeout(() => {
    if (typeof obiraEnhanceCart === 'function') obiraEnhanceCart();
  }, 0);
});
